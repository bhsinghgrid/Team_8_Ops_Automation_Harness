...
from google.generativeai import types # Corrected import for types
...
class RLMClient:
    def __init__(self):
        ...
        self.client = genai.GenerativeModel(model_name='gemini-2.5-flash') if self.api_key else None
    ...
    async def process(self, user_prompt: str, system_instruction: str = "You are a helpful AI assistant. Respond in JSON format.", preset: str = "medium") -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"❌ [RLMClient ERROR]: JSON decoding failed: {e}. Raw response: {response.text}")
            return {
                "final_result": f"RLM JSON decoding failure: {e}. Raw: {response.text[:200]}...",
                "confidence": 0.0,
                "decomposed_tasks": [],
                "cache_hits": 0,
                "retry_count": 0
